    ...
    <table class="table is-fullwidth">
      ...
      <tbody>
        <tr v-for="item in items" :key="item.id + item.size">
          ...
          <td v-if="!viewOnly"><input type="number" min="1" v-model="item.quantity" @change="editQuantity(item.id, item.size, $event.target.value)" class="input" /></td>
          <td v-if="viewOnly">{{ item.quantity }}</td>
          <td>{{ item.price | price }}</td>
          <td>{{ item.price * item.quantity | price }}</td>
          <td v-if="!viewOnly"><a @click="removeItem(item.id, item.size)"><fa-icon icon="times" /></a></td>
        </tr>
      </tbody>
      <tfoot>
        <tr>
